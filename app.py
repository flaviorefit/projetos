...
def convert_df_to_excel(df):
    output = io.BytesIO()
    # Remove timezone information from datetime columns before exporting to Excel
...
# SESSION STATE
# =======================
if "login_realizado" not in st.session_state: st.session_state["login_realizado"] = True
if "usuario_logado" not in st.session_state: st.session_state["usuario_logado"] = "suprimentos"
# A verificação de 'user_role' foi removida por não ser mais necessária.
...
# =======================
# Menu fixo com acesso total, conforme solicitado.
...
if aba=="Dashboard":
    ...
    if not df_filtrado.empty:
        ...
        # Gráfico 1: Quantidade por Status
        ...
        # Gráfico 2: Quantidade por Responsável
        if 'Responsavel' in df_filtrado and not df_filtrado['Responsavel'].dropna().empty:
            ...
            fig_resp = px.bar(
                ...
                text_auto=True, 
                title='Quantidade de Projetos por Responsável'
            )
    ...
    if df_gantt.empty:
        ...
        fig = px.timeline(
            ...
            x_start="Data_Inicio", 
            x_end="Data_Termino", 
            y="Atividades_Descricao", 
            color="Status",
            color_discrete_map=mapa_de_cores,
            title="Linha do Tempo dos Projetos (Gráfico de Gantt)",
            hover_data=["Responsavel", "Atividades_Descricao", "Status"]
        )
        ...
        fig.update_traces(
            hovertemplate="<br>".join([
                ...
                "<b>Responsável:</b> %{customdata[0]}",
                "<b>Início:</b> %{base|%d/%m/%Y}",
                "<b>Fim:</b> %{x[1]|%d/%m/%Y}",
                "<extra></extra>"
            ])
        )
    ...
    st.dataframe(df_filtrado, use_container_width=True, hide_index=True,
        column_config={"Link_dos_Arquivos": st.column_config.LinkColumn("Link dos Arquivos", display_text="Abrir ↗")}
    )
...
# =======================
    ...
    with st.form("form_cadastro", clear_on_submit=True):
        ...
        if submitted:
            projeto_dict = {
                "ID_Projeto": novo_id, "Id_Contrato": id_contrato, "Requisicao": requisicao,
                "Area_Setor": area_setor, "Categoria": categoria, "Empresa": empresa,
                "Responsavel": responsavel, "Atividades_Descricao": descricao, 
                "Link_dos_Arquivos": link_arquivos, "Status": status,
                "Tem_Budget": tem_budget, "Tem_Baseline": tem_baseline,
                "Budget": budget, "Baseline": baseline, "Melhor_Proposta": melhor_proposta,
                "Preco_Inicial": preco_inicial, "Preco_Final": preco_final,
                "Data_Inicio": pd.to_datetime(data_inicio), "Data_Termino": pd.to_datetime(data_termino)
            }
    ...
    if id_selecionado:
        ...
        if projeto:
            ...
            with st.form("form_atualizar"):
                ...
                if submitted:
                    update_data = {
                        "Id_Contrato": id_contrato, "Requisicao": requisicao, "Area_Setor": area_setor, "Categoria": categoria,
                        "Empresa": empresa, "Responsavel": responsavel, "Atividades_Descricao": descricao,
                        "Link_dos_Arquivos": link_arquivos, "Status": status,
                        "Tem_Budget": tem_budget_upd, "Tem_Baseline": tem_baseline_upd,
                        "Budget": budget, "Baseline": baseline, "Melhor_Proposta": melhor_proposta,
                        "Preco_Inicial": preco_inicial, "Preco_Final": preco_final,
                        "Data_Inicio": pd.to_datetime(data_inicio), "Data_Termino": pd.to_datetime(data_termino)
                    }
